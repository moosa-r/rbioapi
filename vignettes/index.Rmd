---
title: "1: A User-friendly R Interface to Biologic web Services's API"
...
vignette: >
  %\VignetteIndexEntry{1: User-friendly Interface to Biologic web Services}
...
rbioapi is an interface to Biological databases and web services. The goal of rbioapi is to provide a user-friendly and unified interface to biological databases, to insulate the user from technicalities when it comes to using API services and to create a unified, easy-to-learn, and easy-to-implement way to connect to biologic databases.

With rbioapi, You are not required to to have any knowledge about HTTP protocols, CURL, REST API, parsing the response, etc. **Just fill a function's arguments and the rest is handled for you.**
...
There are three exceptions: `rba_options()`, `rba_connection_test()`, and `rba_pages()`. more on that later.
