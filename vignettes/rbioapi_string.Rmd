...
`ids = NULL` and supply both `network_term_id` and `species`. For example,
`GO:0050852` identifies the GO term "T cell receptor signaling pathway":
...
Term identifiers can also be obtained from the `term` column returned by
`rba_string_enrichment()`. The `ids` and `network_term_id` input modes cannot
be combined.
