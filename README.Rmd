...
[![CRAN version](https://www.r-pkg.org/badges/version/rbioapi)](https://cran.r-project.org/package=rbioapi)
...
> **Development version**
> The project documentation reflects the current development version of rbioapi and may differ from the latest CRAN release. For documentation matching the CRAN version, see the CRAN reference manual or the help pages installed with the package.
...
1.  [Enrichr](https://maayanlab.cloud/Enrichr/ "Enrichr") ([rbioapi vignette article](https://rbioapi.moosa-r.com/articles/rbioapi_enrichr.html "rbioapi & Enrichr vignette article"))
2.  [JASPAR](https://jaspar.elixir.no/ "JASPAR - A database of transcription factor binding profiles") ([rbioapi vignette article](https://rbioapi.moosa-r.com/articles/rbioapi_jaspar.html "rbioapi & Enrichr vignette article"))
3.  [miEAA](https://ccb-compute2.cs.uni-saarland.de/mieaa/ "miRNA Enrichment Analysis and Annotation Tool (miEAA)") ([rbioapi vignette article](https://rbioapi.moosa-r.com/articles/rbioapi_mieaa.html "rbioapi & miEAA vignette article"))
...
There are four exceptions: `rba_options()`, `rba_connection_test()`, `rba_pages()`, and `rba_metadata()`; these are helper functions. More on that later.
...
The `rba_connection_test()` helper checks your internet connection and whether supported services and databases are available. If you encounter an error while using rbioapi, run this function to check your internet connection and the availability of supported services.
...
Some API resources return paginated responses, particularly when a query can produce many records. Their rbioapi functions generally expose a page argument, such as `page_number`, and may also expose a page-size argument. `rba_pages()` repeats one quoted rbioapi call for the pages you request.
...
As you can see, the server has returned the first page of the response. To retrieve multiple pages, wrap the function call in `quote()` and replace its named page argument with a specification of the form `"pages:start:end"`. For example, the following call retrieves pages 1 to 3:
...
There is another, functionally equivalent, way to call `rba_pages()`: omit the page argument from the quoted call, then supply its exact name and the desired page numbers separately. This form is particularly useful when you want to provide a vector of page numbers that does not necessarily form a contiguous range:

```{r rba_pages3, echo=TRUE, eval=FALSE}
adeno_pages <- rba_pages(
  input_call = quote(
    rba_uniprot_taxonomy_name(
      name = "adenovirus",
...
rbioapi can store information about the API requests used to create a result as an attribute of the returned object. Metadata collection is off by default. Set `metadata = TRUE` for one call, then use `rba_metadata()` to get it:
...
The returned `rba_metadata` object prints a short summary and can be used like a regular list:

```{r rba_metadata_access}
...
To save metadata with all later rbioapi calls, use `rba_options(metadata = TRUE)`. Turn it off again with `rba_options(metadata = FALSE)`. For a result without metadata, `rba_metadata()` returns `NULL`.

Saving the complete `httr` responses and parser functions can make results and saved files much larger.
...
-   [How to cite miEAA](https://rbioapi.moosa-r.com/articles/rbioapi_mieaa.html#citations "How to cite miEAA"). (See the [miEAA website](https://ccb-compute2.cs.uni-saarland.de/mieaa/).)
