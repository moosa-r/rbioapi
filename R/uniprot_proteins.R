...
rba_uniprot_proteins_search = function(accession = NA,
  ...
  v_msg("Searching UniProt and retrieving proteins that match your provided inputs.")
...
rba_uniprot_proteins = function(accession,
  ...
  v_msg("Retrieving %sUniProt Entity with accession number %s.",
...
rba_uniprot_proteins_crossref = function(db_id,
  ...
  rba_ba_ext_args(...)
  ## Check User-input Arguments
  rba_ba_args(cons = list(list(arg = "db_name",
                               class = "character"),
                          list(arg = "db_id",
                               class = "character"),
                          list(arg = "reviewed",
                          ...
                          list(arg = "isoform",
                               class = "numeric",
                               val = c(0, 1, 2))))
  v_msg("Retrieving UniProt entities that correspond to ID %s in database %s.",
        db_id, db_name)
  ## Build GET API Request's query
  call_query = rba_ba_query(init = list("size" = "-1"),
                            list("reviewed",
...
rba_uniprot_features_search = function(accession = NA,
  ...
  v_msg("Searching UniProt and retrieving sequence annotations (features) of proteins that match your provided inputs.")
  ## Build GET API Request's query
  call_query = rba_ba_query(init = list("size" = "-1"),
                            list("accession",
                                 any(!is.na(accession)),
...
rba_uniprot_features = function(accession,
  ...
  v_msg("Retrieving sequence annotations (features) of protein %s.", accession)
  ## Build GET API Request's query
  call_query = rba_ba_query(init = list("size" = "-1"),
                            list("categories",
                                 any(!is.na(categories)),
...
rba_uniprot_variation_search = function(accession = NA,
  ...
  v_msg("Searching UniProt and retrieving natural variations of proteins that match your provided inputs.")
  ## Build GET API Request's query
  call_query = rba_ba_query(init = list("size" = "-1"),
                            list("accession",
                                 any(!is.na(accession)),
  ...
  save_to = ifelse(isFALSE(save_peff),
                   yes = rba_ba_file(file = "uniprot_variation.json"),
                   no = rba_ba_file(file = "uniprot_variation.peff",
                                    save_to = save_peff))
...
rba_uniprot_variation = function(id,
  ...
  v_msg("Retrieving Natural variant of %s.",
  ...
  save_to = ifelse(isFALSE(save_peff),
                   yes = rba_ba_file(file = file_name),
                   no = rba_ba_file(file = file_name,
                                    save_to = save_peff))
  path_input = switch(id_type,
                      "uniprot" = paste0(rba_ba_stg("uniprot", "pth"),
                                         "variation/", id),
                      "hgvs" = paste0(rba_ba_stg("uniprot", "pth"),
                                      "variation/hgvs/", id),
                      "dbsnp" = paste0(rba_ba_stg("uniprot", "pth"),
                                       "variation/dbsnp/", id))
...
rba_uniprot_proteomics_search = function(accession = NA,
  ...
  v_msg("Searching UniProt and retrieving proteomics peptides features of proteins that match your provided inputs.")
  ## Build GET API Request's query
  call_query = rba_ba_query(init = list("size" = "-1"),
                            list("accession",
                                 any(!is.na(accession)),
...
rba_uniprot_proteomics = function(accession,
  ...
  v_msg("Retrieving proteomics peptides features mapped to the sequence of protein %s.",
        accession)
  ## Build Function-Specific Call
  input_call = rba_ba_httr(httr = "get",
                           url = rba_ba_stg("uniprot", "url"),
...
rba_uniprot_antigen_search = function(accession = NA,
  ...
  v_msg("Searching UniProt and retrieving antigenic features of proteins that match your provided inputs.")
  ## Build GET API Request's query
  call_query = rba_ba_query(init = list("size" = "-1"),
                            list("accession",
                                 any(!is.na(accession)),
...
rba_uniprot_antigen = function(accession,
  ...
  v_msg("Retrieving Antigenic features mapped to the sequence of protein %s.",
        accession)
  ## Build Function-Specific Call
  input_call = rba_ba_httr(httr = "get",
                           url = rba_ba_stg("uniprot", "url"),
