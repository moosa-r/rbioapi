...
[![CRAN
version](https://www.r-pkg.org/badges/version/rbioapi)](https://cran.r-project.org/package=rbioapi)
[![R-CMD-check](https://img.shields.io/github/actions/workflow/status/moosa-r/rbioapi/R-CMD-check.yaml?branch=master&event=push&label=R-CMD-check&logo=R)](https://github.com/moosa-r/rbioapi/actions/workflows/R-CMD-check.yaml)
...
There are four exceptions: `rba_options()`, `rba_connection_test()`,
`rba_pages()`, and `rba_metadata()`; these are helper functions. More on
that later.
...
The `rba_connection_test()` helper checks your internet connection and
whether supported services and databases are available. If you encounter
an error while using rbioapi, run this function to check your internet
connection and the availability of supported services.
...
argument. `rba_pages()` repeats one quoted rbioapi call for the pages
you request.
...
To retrieve multiple pages, wrap the function call in `quote()` and
replace its named page argument with a specification of the form
...
There is another, functionally equivalent, way to call `rba_pages()`:
...
#>     2026-08-29 01:04:47 GMT | GET | HTTP 200 OK | parsers used: 1
#> 
...
#> List of 1
#>  $ :List of 4
#>   ..$ timestamp: POSIXct[1:1], format: "2026-08-29 01:04:47"
#>   ..$ call     : language httr::GET(url = "https://reactome.org", path = "ContentService/data/species/all",      httr::user_agent(getOption| __truncated__ ...
#>   ..$ response :List of 10
...
# Session info
    ...
    #>  [9] compiler_4.6.0    httr_1.4.8        rstudioapi_0.19.0 tools_4.6.0      
    #> [13] curl_7.1.0        evaluate_1.0.5    yaml_2.3.12       otel_0.2.0       
    #> [17] jsonlite_2.0.0    rlang_1.3.0
